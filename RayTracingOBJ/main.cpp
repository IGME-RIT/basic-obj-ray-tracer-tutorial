...
void loadOBJ(Mesh* m)
{
	...
	for (int i = 0; i < (int)numVerts / 3; i++)
	{
			...
			for (int k = 0; k < 3; k++)
			{
				...
				m->triangles[i].a[k] = pos[coordIndex];
			}

			for (int k = 0; k < 3; k++)
			{
				int normalIndex = 3 * faces[9 * i + 3 * j + 2] + k;
				normal0[k] = norms[normalIndex];
			}
			...
			{
				int coordIndex = 3 * faces[9 * i + 3 * j + 0] + k;
				m->triangles[i].b[k] = pos[coordIndex];
			}

			for (int k = 0; k < 3; k++)
			{
				int normalIndex = 3 * faces[9 * i + 3 * j + 2] + k;
				normal1[k] = norms[normalIndex];
			}
			...
			{
				int coordIndex = 3 * faces[9 * i + 3 * j + 0] + k;
				m->triangles[i].c[k] = pos[coordIndex];
			}

			for (int k = 0; k < 3; k++)
			{
				int normalIndex = 3 * faces[9 * i + 3 * j + 2] + k;
				normal2[k] = norms[normalIndex];
			}

			glm::vec3 averageNormal = normal0 + normal1 + normal2;
			averageNormal /= 3;
			averageNormal = normalize(averageNormal);

			m->triangles[i].color = glm::vec4(0.2, 1.0, 0.2, 1.0);
			m->triangles[i].normal = glm::vec4(averageNormal, 1.0);
